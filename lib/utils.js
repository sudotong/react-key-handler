...
function elmntOrArrayMatch<T>(a: T | T[], b: T) {
  if (Array.isArray(a)) {
    return a.includes(b);
  }
  return a === b;
...
export function matchesKeyboardEvent(event: KeyboardEvent, {keyCode, keyValue, code}: KeyboardKey): boolean {
  console.log('asdasd', keyValue, keyCode);
  if (!isNullOrUndefined(keyValue)) {
    const value = eventKey(event);
    if (elmntOrArrayMatch(keyValue, value)) {
      return true;
    }
  }

  if (!isNullOrUndefined(code)) {
    if (elmntOrArrayMatch(code, event.code)) {
      return true;
    }
  }
  ...
  if (!isNullOrUndefined(keyCode)) {
    ...
    if (elmntOrArrayMatch(keyCode, keyCodeTarget)) {
      return true;
    }
  }
